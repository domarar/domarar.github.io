...
if (Array.isArray(section.topics)) {
    ...
    for (let i = 0; i < topics.length; i++) {
        ...
        const isTeamOfficialsParent =
            law.number === 12 &&
            topic.title.trim().replace(/\.$/, "") === "Forráðamenn liðs";
        ...
        if (isTeamOfficialsParent) {
            ...
            while (
                j < topics.length &&
                subtopicTitles.includes(
                    topics[j].title.trim().replace(/\.$/, "")
                )
            ) {
...
function renderTopicGroup(parentTopic, subtopics) {
    ...
    const parentContent =
        renderTopicContent(parentTopic);
