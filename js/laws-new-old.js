...
function renderSingleLaw(lawNumber, options = {}) {
    ...
    <div class="law-top-navigation-right">
        <button
            class="law-top-nav-button law-top-nav-previous"
            type="button"
            ${Number(law.number) === 1 ? "disabled" : ""}
        >
            ← Fyrri grein
        </button>

        <button
            class="law-top-nav-button law-top-nav-next"
            type="button"
            ${Number(law.number) === 17 ? "disabled" : ""}
        >
            Næsta grein →
        </button>
    </div>
...
function setupSingleLawEvents() {
    ...
    document.querySelector(".law-top-nav-previous");

const nextButton =
    document.querySelector(".law-top-nav-next");

if (previousButton) {
    previousButton.addEventListener("click", () => {
        const currentLawNumber = Number(
            document.querySelector(".single-law-number")?.textContent
        );

        if (currentLawNumber > 1) {
            renderSingleLaw(currentLawNumber - 1);
        }
    });
}
