...
COMPETITIONS_2023 = [
    ...
    {
    "id": 190224,
    "name": "Besta deild karla 2023",
    "gender": "Male",
    "type": "Deild",
},
    {
        "id": 190220,
        "name": "Besta deild karla 2023 - Neðri hluti",
        "gender": "Male",
        "type": "Deild",
    },
    {
        "id": 190226,
        "name": "Besta deild karla 2023 - Efri hluti",
        "gender": "Male",
        "type": "Deild",
    },
        ...
        {
        "id": 190223,
        "name": "Lengjudeild karla 2023",
        "gender": "Male",
        "type": "Deild",
    },
    {
        "id": 190034,
        "name": "Lengjudeild karla 2023 - Umspil",
        "gender": "Male",
        "type": "Deild",
    },
    {
        "id": 190219,
        "name": "2. deild karla 2023",
        "gender": "Male",
        "type": "Deild",
    },
    {
        "id": 190227,
        "name": "3. deild karla 2023",
        "gender": "Male",
        "type": "Deild",
    },
    {
        "id": 190174,
        "name": "4. deild karla 2023",
        "gender": "Male",
        "type": "Deild",
    },
    {
        "id": 190222,
        "name": "Besta deild kvenna 2023",
        "gender": "Female",
        "type": "Deild",
    },
    {
        "id": 190176,
        "name": "Besta deild kvenna 2023 - Neðri hluti",
        "gender": "Female",
        "type": "Deild",
    },
    {
        "id": 190172,
        "name": "Besta deild kvenna 2023 - Efri hluti",
        "gender": "Female",
        "type": "Deild",
    },
    {
        "id": 190229,
        "name": "Lengjudeild kvenna 2023",
        "gender": "Female",
        "type": "Deild",
    },
]
...
for competition in COMPETITIONS_2023:
...
# Reuse officials already fetched in the previous progress file
progress_path = "data/archive-2023-progress.json"

try:
    with open(progress_path, "r", encoding="utf-8") as progress_file:
        previous_progress = json.load(progress_file)

    previous_matches = {
        match["id"]: match
        for match in previous_progress.get("games", [])
    }

    reused_count = 0

    for match in parsed_matches:
        previous_match = previous_matches.get(match["id"])

        if previous_match and previous_match.get("officials"):
            match["officials"] = previous_match["officials"]
            reused_count += 1

    print(f"Reused officials for {reused_count} matches")
...
for index, match in enumerate(parsed_matches, start=1):
    ...
    if match.get("officials"):
        print(
            f"{index}/{len(parsed_matches)} - "
            f"{match_id}: already fetched - skipping"
        )
        continue
    print(
        f"{index}/{len(parsed_matches)} - "
    ...
    try:
        ...
        with open(
         "data/archive-2023-progress.json",
         "w",
         encoding="utf-8"
         ) as progress_file:
          json.dump(
...
print(
    ...
    f"to data/archive-2023-test.json"
)       
